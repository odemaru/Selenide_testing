...
| Способ выбран | Поля способа | Заполнены | Исход |
|---|---|---|---|
| нет | — | — | подтверждение заблокировано |
| да | есть | нет | платёж не принимается |
| да | есть | да | платёж принимается |
| да | нет вовсе | — | подтверждение доступно |

Вторая строка проверяет исход, а не механизм, и это осознанно. Публичный
стенд добивается его блокировкой кнопки, а образ, на котором прогон идёт
в CI, собран раньше и позволяет нажать — платёж всё равно не проходит.
Проверка «кнопка заблокирована» зеленела бы только на одной из версий.
